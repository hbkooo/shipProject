...
def get_rroidb():	
	...
	L_ICDAR2003_TRAIN = get_ICDAR2017_mlt("train", "multi_class", False)
...
def get_ICDAR2017_mlt(mode, task, prefetched):
	...
	DATASET_DIR = '/home/xw/RRPN/data/ships/'
	...
	cls_list = {'background':0, 'warship':1, 'civil-ship':2, 'submarine':3, 'aircraft-carrier':4}
        if False:
        #if not prefetched:
            if mode == "train":
                for filename in os.listdir(DATASET_DIR + "train/JPEGImages"):
                    img_candidate_path = DATASET_DIR + "train/JPEGImages/" + filename
		    im = Image.open(img_candidate_path)
		    im = im.convert('RGB')
		    im.save(DATASET_DIR + "train/JPEGImages/" + filename.split('.')[0] + ".jpg")
		    img_candidate_path = DATASET_DIR+ "train/JPEGImages/" + filename.split('.')[0] +".jpg"
		    data_list.append(img_candidate_path)
			
		    gt_candidate_path = DATASET_DIR + "train/Annotations_txt/" + filename.split('.')[0] + ".txt"	
			...
			if len(gt_ind) > 3:
				...
				height = edge2
				if pt1[0] - pt2[0] !=0:
                                    angle = -np.arctan(float(pt1[1] - pt2[1]) / float(pt1[0] - pt2[0])) / math.pi * 180
				else:
                                    angle = 90.0
			    elif edge2 >= edge1:
                                width = edge2
				height = edge1
				if pt2[0] - pt3[0] != 0:
                                    angle = -np.arctan(float(pt2[1] - pt3[1]) / float(pt2[0] - pt3[0])) / math.pi * 180
                                else:
				    angle = 90.0
			    if angle < -45.0:
                                angle = angle + 180
		...
		print ("Save pickle done.")						
                ...
                for filename in os.listdir(DATASET_DIR + "val/JPEGImages"):
		    img_candidate_path = DATASET_DIR + "val/JPEGImages/" + filename
		    im = im.convert('RGB')
		    im.save(DATASET_DIR + "val/JPEGImages/" + filename.split('.')[0] + ".jpg" )  
		    img_candidate_path = DATASET_DIR + "val/JPEGImages/" + filename.split('.')[0] +".jpg"
		    data_list.append(img_candidate_path)
				#print ("data_list:", len(data_list))	
			
		    gt_candidate_path = DATASET_DIR + "val/Annotations_txt/" + filename.split('.')[0] + ".txt"	
		    if os.path.isfile(gt_candidate_path):
			gt_list.append(gt_candidate_path)	
        ...
        if True:
            if mode == "train":
		f_pkl = open('/home/xw/RRPN/ICDAR2017_training_cache.pkl', 'rb')
